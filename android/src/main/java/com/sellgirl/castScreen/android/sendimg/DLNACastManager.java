...
public class DLNACastManager //implements IDLNADeviceCaster
{
    ...
    public void startStreaming2(Activity activity, DLNADeviceScanner2.DLNADevice device, UpnpService upnpService) {
        if(!inited) {
            inited=true;
            // 1. 创建 HTTP 服务器
            try {
...
// 获取本机 IP，构造图片 URL
        String localIp = getLocalIpAddress(); // 你之前实现的方法
        String imageUrl = "http://" + localIp + ":8080/image.jpg";

// 投屏图片
        castImage(imageUrl, "Test Image");
    }
...
//    }
    public void castImage(String imageUrl, String title) {
        if (controlPoint == null || targetDevice == null) {
            Log.e("CastManager", "Not connected");
            return;
        }
        ...
        Service avTransport = targetDevice.findService(avType);
        if (avTransport == null) {
            Log.e("CastManager", "AVTransport not found");
            return;
        }

        // 构建 DIDL，类型设为 ImageItem（而不是 VideoItem）
        DIDLContent didl = new DIDLContent();
        org.jupnp.support.model.item.ImageItem imageItem = new org.jupnp.support.model.item.ImageItem();
        imageItem.setId("0");
        imageItem.setParentID("0");
        imageItem.setTitle(title);
        imageItem.setCreator("Test");

        // 使用 image/jpeg MIME 类型
        MimeType mimeType = new MimeType("image", "jpeg");
        Res res = new Res(mimeType, null, imageUrl);
        imageItem.addResource(res);
        didl.addItem(imageItem);

        // 发送 SetAVTransportURI 指令（和视频完全一样）
        ActionInvocation setUriInvocation = new ActionInvocation(
            avTransport.getAction("SetAVTransportURI")
        );
        setUriInvocation.setInput("InstanceID", 0);
        setUriInvocation.setInput("CurrentURI", imageUrl);
        setUriInvocation.setInput("CurrentURIMetaData", didl.toString());

        controlPoint.execute(new ActionCallback(setUriInvocation) {
            @Override
            public void success(ActionInvocation invocation) {
                Log.d("CastManager", "SetAVTransportURI (Image) success");
                // 发送 Play 指令
                ActionInvocation playInvocation = new ActionInvocation(
                    avTransport.getAction("Play")
                );
                playInvocation.setInput("InstanceID", 0);
                playInvocation.setInput("Speed", "1");
                controlPoint.execute(new ActionCallback(playInvocation) {
                    @Override
                    public void success(ActionInvocation invocation) {
                        Log.d("CastManager", "Play (Image) success");
                    }
                    @Override
                    public void failure(ActionInvocation invocation, UpnpResponse operation, String defaultMsg) {
                        Log.e("CastManager", "Play (Image) failed: " + defaultMsg);
                    }
                });
            }
            @Override
            public void failure(ActionInvocation invocation, UpnpResponse operation, String defaultMsg) {
                Log.e("CastManager", "SetAVTransportURI (Image) failed: " + defaultMsg);
            }
        });
    }
}
